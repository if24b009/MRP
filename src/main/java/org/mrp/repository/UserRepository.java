...
public class UserRepository implements Repository<User> {
    ...
    public ResultSet findByToken(String token) throws SQLException {
        return db.query("SELECT user_id FROM token WHERE token = ?", token);
    }
    ...
    public void updateUsername(UUID userId, String username) throws SQLException {
        db.update("UPDATE app_user SET username = ? WHERE user_id = ?", username, userId);
    }
    ...
    public int getRatings_total(UUID userId) throws SQLException {
        Object ct = db.getValue("SELECT COUNT(*) FROM rating WHERE user_id = ?", userId);
        return ct != null ? ((Number) ct).intValue() : 0;
    }

    public int getFavorites_total(UUID userId) throws SQLException {
        Object ct = db.getValue("SELECT COUNT(*) FROM favorite WHERE user_id = ?", userId);
        return ct != null ? ((Number) ct).intValue() : 0;
    }

    public int getMediaEntriesCreated_total(UUID userId) throws SQLException {
        Object ct = db.getValue("SELECT COUNT(*) FROM media_entry WHERE creator_id = ?", userId);
        return ct != null ? ((Number) ct).intValue() : 0;
    }

    public double getAvgScore(UUID userId) throws SQLException {
        Object avg = db.getValue("SELECT COALESCE(AVG(stars_ct), 0) FROM rating WHERE user_id = ?", userId);
    ...
    public List<MediaEntry> findRecommendations(UUID userId, List<Genre> favoriteGenres, List<MediaEntryType> favoriteMediaTypes, List<Integer> preferredAgeRestrictions) throws SQLException {
        ...
        StringBuilder filterCondition = new StringBuilder(); //StringBuilder = efficient way to build string
        //filterCondition = added WHERE-Klausel
        ...
        if (!favoriteGenres.isEmpty()) appendINClause("meg.genre", favoriteGenres, filterCondition, filterParams);

        //Media Entry Type: Build MediaType-WHERE-Klausel
        if (!favoriteMediaTypes.isEmpty()) appendINClause("m.type", favoriteMediaTypes, filterCondition, filterParams);

        //Age Restriction: Build AgeRestriction-WHERE-Klausel
        if (!preferredAgeRestrictions.isEmpty()) appendINClause("m.age_restriction", preferredAgeRestrictions, filterCondition, filterParams);

        ResultSet rs = getRecommendationsBasedOnConditions(filterCondition, filterParams);
    ...
    //Helperfunction: Build sql-filter condition Klausel, which is added in WHERE-Klausel in sql-statement
    private <T> void appendINClause(String columnName, List<T> values, StringBuilder filterCondition, List<Object> filterParams) {
        if (values == null || values.isEmpty()) return;

        if(!filterCondition.isEmpty()) filterCondition.append(" OR ");

        filterCondition.append(columnName).append(" IN (");
        for (int i = 0; i < values.size(); i++) {
            if (i > 0) filterCondition.append(", ");
    ...
    //Helperfunction: execute sql query with finished filter-condition, which is filled with the params-list
    private ResultSet getRecommendationsBasedOnConditions(StringBuilder condition, List<Object> params) throws SQLException {
